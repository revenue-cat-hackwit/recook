        ...
        {SUPPORTED_LANGUAGES.map((lang) => (
          <TouchableOpacity
            ...
            className={`rounded-full px-3 py-1.5 ${
              language === lang.code ? 'bg-[#CC5544]' : 'bg-gray-200'
            }`}
          >
      ...
      {SUPPORTED_LANGUAGES.map((lang) => (
        <TouchableOpacity
          ...
          className={`flex-row items-center justify-between rounded-2xl border p-4 ${
            language === lang.code ? 'border-[#CC5544] bg-red-50' : 'border-gray-200 bg-white'
          }`}
        >
          <View className="flex-row items-center gap-3">
            ...
            <Text
              className={`font-visby text-base ${
                language === lang.code ? 'text-[#CC5544]' : 'text-gray-700'
              }`}
            >
              {lang.label}
            </Text>
          ...
          {language === lang.code && (
            <View className="h-5 w-5 items-center justify-center rounded-full bg-[#CC5544]">
